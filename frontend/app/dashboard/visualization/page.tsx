...
export default function VisualizationPage() {
  ...
  const fetchDatasets = useCallback(async () => {
    try {
      ...
      // Select the most recent dataset (latest uploaded)
  ...
  // Fetch visualization data for a specific dataset
  const fetchVisualizationData = async (datasetId: number) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8000/api/datasets/${datasetId}/data`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  // Process raw data into visualization-friendly format
  const processDataForVisualization = (data: any[]): VisualizationData => {
    // Sales by month (using Purchased as sales indicator)
    ...
    return {
      ...
      ageDistribution,
      topProducts: [
        { name: 'Product A', sales: Math.round(totalRevenue * 0.35) },
        { name: 'Product B', sales: Math.round(totalRevenue * 0.25) },
        { name: 'Product C', sales: Math.round(totalRevenue * 0.20) },
        { name: 'Product D', sales: Math.round(totalRevenue * 0.20) },
      ],
      kpis: {
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5 border border-green-200">
              ...
              <p className="text-xs text-gray-500 mt-1">From {visualizationData.kpis.totalCustomers} customers</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-5 border border-orange-200">
              ...
              <p className="text-xs text-gray-500 mt-1">Sales to leads ratio</p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-sm text-gray-500 mb-1">Total Income (All Customers)</p>
                <p className="text-xl font-bold text-gray-900">{visualizationData.kpis.totalIncome}</p>
              </div>
              <div className="text-center">
                <p className="text-sm text-gray-500 mb-1">Average Customer Age</p>
                <p className="text-xl font-bold text-gray-900">{visualizationData.kpis.avgAge} years</p>
              </div>
              <div className="text-center">
                <p className="text-sm text-gray-500 mb-1">Data Quality Score</p>
      ...
      {!loadingData && datasets.length > 0 && !visualizationData && (
        <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-gray-500">
            The selected dataset has no data to visualize.
          </p>
          <button
            onClick={handleRefresh}
